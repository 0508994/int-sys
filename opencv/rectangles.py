...
def detect_rects(path, min_t, max_t):
    ...
    for cnt in contours:
        #       In this, second argument is called epsilon, which is maximum distance from contour
        #       to approximated contour. It is an accuracy parameter.
...
"""
    ...
    we’ll be computing the Euclidean distance between our dataset of known colors (i.e., the lab
    array) and the averages of a particular image region. The known color that minimizes
    the Euclidean distance will be chosen as the color identification. And unlike HSV and RGB
    color spaces, the Euclidean distance between L*a*b* colors has actual perceptual meaning.
"""
...
def main():
    ...
    detect_spec_rects(
        args.path, int(args.min), int(args.max), args.color, float(args.size)
    )
